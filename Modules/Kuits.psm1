...
#region <GET-TIMFILEMISSINGTIME>

##############################################################################
#.SYNOPSIS
# Creates new TIM file from failures
# 
#.DESCRIPTION
# Query's each line in a failed TIM file and creates a new file for each entry that does not exist in TimeTransactions in Partner DB
#
#.PARAMETER Location
# Failed, Working or Pending
#
#.EXAMPLE
# Get-TimFileMissingTime -Location Failed
##############################################################################
...
#region <GET-MISSINGPHOTO>


##############################################################################
#.SYNOPSIS
# Finds staff with no photos
# 
#.DESCRIPTION
# Find's staff with no AD hopoto and displays them in a grid
#
#.EXAMPLE
# Get-MissingPhoto
##############################################################################

Function Get-MissingPhoto
    {
    Get-ADUser -Filter * -SearchBase "ou=ksl staff,dc=kuits,dc=local" -Properties thumbnailPhoto | Where-Object {$_.thumbnailPhoto -eq $Null -and $_.distinguishedname -notlike "*generic*" -and $_.name -notlike "test*" -and $_.name -notlike "laptop*" -and $_.name -notlike "Pv4*" -and $_.name -notlike "Gillian Battersby*" -and $_.name -notlike "Karen Birtwistle*" -and $_.name -notlike "Lorraine McCau*" -and $_.name -notlike "Sarah Lan*"} | select name |  sort name | out-gridview
    }

#endregion

#region <GET-CALENDARPERMISSIONS>

##############################################################################
#.SYNOPSIS
# Returns calendar permissions
# 
#.DESCRIPTION
# Returns calendar poermissions for the specified user
#
#.EXAMPLE
# Get-CalendarPermissions -Name "Wesley Stott"
##############################################################################
...
#region <ADD-CALENDARPERMISSIONS>

##############################################################################
#.SYNOPSIS
# Adds calendar permissions
# 
#.DESCRIPTION
# Adds calendar poermissions for the specified user
#
#.EXAMPLE
# Get-CalendarPermissions -Name "Wesley Stott" -Requester "James Harris"
##############################################################################
...
#region <REMOVE-CALENDARPERMISSIONS>

##############################################################################
#.SYNOPSIS
# Removes calendar permissions
# 
#.DESCRIPTION
# Removes calendar poermissions for the specified user
#
#.EXAMPLE
# Remove-CalendarPermissions -Name "Wesley Stott" -Requester "James Harris"
##############################################################################
